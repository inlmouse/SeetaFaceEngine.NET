...
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width, int im_height, seeta::FaceInfo face_loc, float *facial_loc)
{
  ...
  int extend_lx = std::max(floor(left_x - extend_factor*bbox_w), 0.0f);
  int extend_rx = std::min(floor(right_x + extend_factor*bbox_w), (im_width - 1)*1.0f);
  int extend_ly = std::max(floor(left_y - (extend_factor - extend_revised_y)*bbox_h), 0.0f);
  int extend_ry = std::min(floor(right_y + (extend_factor + extend_revised_y)*bbox_h), (im_height - 1)*1.0f);
